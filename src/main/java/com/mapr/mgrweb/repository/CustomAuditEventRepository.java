...
import com.mapr.mgrweb.config.audit.AuditEventConverter;
...
public class CustomAuditEventRepository implements AuditEventRepository {
    ...
    private final MapRAuditEventRepository persistenceAuditEventRepository;

    private final AuditEventConverter auditEventConverter;

    private final Logger log = LoggerFactory.getLogger(getClass());

    public CustomAuditEventRepository(MapRAuditEventRepository persistenceAuditEventRepository, AuditEventConverter auditEventConverter) {
    ...
    public void add(AuditEvent event) {
        if (!AUTHORIZATION_FAILURE.equals(event.getType()) && !Constants.ANONYMOUS_USER.equals(event.getPrincipal())) {
            ...
            persistentAuditEvent.setId(UUID.randomUUID().toString());
